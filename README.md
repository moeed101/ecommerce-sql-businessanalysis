...
Retention was analyzed by tracking the percentage of customers active in
subsequent months after acquisition.

Key observations:
- Significant churn occurs after the first month across most cohorts
- Customers who remain active beyond the early months tend to stabilize
- Older cohorts show reactivation during Q4, indicating strong seasonality effects

This analysis highlights the importance of early customer engagement and
seasonal reactivation strategies.

### Cohort Lifetime Value (LTV)
Cohort-based Lifetime Value (LTV) was calculated by accumulating revenue
generated by customers over time since acquisition.

Key observations:
- Lifetime value increases steadily for customers who remain active
- Older cohorts generate substantially higher cumulative value over time
- Q4 periods show notable increases in cumulative LTV due to seasonal demand

This indicates that while many customers churn early, retained customers
deliver increasing long-term value.

### Customer Value Segmentation (RFM)
Customers were segmented using Recency, Frequency, and Monetary (RFM) analysis
to identify high-value and at-risk groups.

Key findings:
- A small percentage of customers qualify as VIPs based on recent activity,
  purchase frequency, and total spend
- These VIP customers represent a disproportionately large share of revenue
- A sizable group of previously active customers shows signs of churn risk

This segmentation enables targeted retention, loyalty, and win-back strategies.

## Notes on Scalability
Some advanced analyses (cohort LTV and RFM segmentation) involve computationally
intensive operations. Logic was validated using representative subsets due to
local database resource constraints.

In a production environment, these analyses would be executed on a cloud data
warehouse or higher-capacity database system.
